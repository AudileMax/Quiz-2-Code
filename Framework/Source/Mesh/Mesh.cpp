...
void Mesh::Draw(ShaderProgram* pShader, Texture* pTexture, fw::Transform* transform, float aspectRatio)
{
    ...
    glUniform2f( u_Scale, transform->m_Scale.x, transform->m_Scale.y );

    GLint u_Angle = glGetUniformLocation(pShader->GetProgram(), "u_Angle");
    glUniform1f( u_Angle, transform->m_Angle );

    GLint u_Offset = glGetUniformLocation(pShader->GetProgram(), "u_Offset");
    glUniform2f( u_Offset, transform->m_Pos.x, transform->m_Pos.y );
